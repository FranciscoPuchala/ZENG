...
- **Cada publicación = un informe INDEPENDIENTE** (corregido ago 2026): varios análisis van en el mismo informe **sólo si se seleccionan y publican juntos** (una misma acción, mismo `analisis_ids`). Si después se ingresa/publica otra muestra del mismo cliente/ensayo/fecha, es un informe **aparte** — NO se suma al anterior. `POST /informes` hace siempre un INSERT nuevo; si el `numero_informe` ya existe, la restricción UNIQUE lo rechaza y el backend devuelve un **409 con mensaje claro** ("cambiá el número"). (Se revirtió el comportamiento acumulativo previo, que juntaba muestras distintas por error.)
