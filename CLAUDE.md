...
- **Un informe agrupa TODAS las sub-muestras de una muestra** (confirmado ago 2026): cuando una muestra tiene varios análisis (m1, m2, m3) del mismo cliente/ensayo/fecha, van **todos en UN solo informe**, no uno por cada uno. Por eso `POST /informes` es **acumulativo**: si se publica con un `numero_informe` que ya existe, se suman esos análisis al informe existente en vez de crear uno nuevo (evita el choque con la restricción UNIQUE de `numero_informe`). Resuelve la duda abierta "si un informe agrupa una o varias muestras".
